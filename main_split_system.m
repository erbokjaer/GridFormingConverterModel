...
function [t, x] = ode4_fixed(odefun, tspan, x0, p)
    dt = tspan(2) - tspan(1);
    t = tspan;
    ...
    for i = 1:(length(t)-1)
        k1 = odefun(t(i),          x(:,i),       p);
        k2 = odefun(t(i) + dt/2,   x(:,i) + dt*k1/2, p);
        k3 = odefun(t(i) + dt/2,   x(:,i) + dt*k2/2, p);
        k4 = odefun(t(i) + dt,     x(:,i) + dt*k3,   p);
...
if update_plots(3)
    ...
    subplot(2,1,2)
    plot(t,50*log.omega_g,'LineWidth',1.2)
    hold on
    plot(t,50*log.omega_conv,'LineWidth',1.2)
    hold off
