function log = reconstruct_logs_6(p,t,x)
...
Econv_d_c = p.Kp_c .* err_id + p.Ki_c .* xi_id ...
            - omega_conv .* p.L1 .* i1q_c - p.L1 .* i1q_c + vd_c;

Econv_q_c = p.Kp_c .* err_iq + p.Ki_c .* xi_iq ...
            + omega_conv .* p.L1 .* i1d_c + p.L1 .* i1d_c + vq_c;

% Econv_d_c = p.Kp_c .* err_id + p.Ki_c .* xi_id ...
%             - omega_conv .* p.L1 .* Iq_ref_lim - p.L1 .* Iq_ref_lim + vd_c;
% 
% Econv_q_c = p.Kp_c .* err_iq + p.Ki_c .* xi_iq ...
%             + omega_conv .* p.L1 .* Id_ref_lim + p.L1 .* Id_ref_lim + vq_c;
...
log.V_meas_c = V_meas;
log.Q_meas_c = Q_meas;
log.P_meas_c = i2d_c .* vd_c + i2q_c .* vq_c;

log.V_meas = sqrt(vd.^2 + vq.^2);
log.Q_meas = log.Q_PCC;
log.P_meas = log.P_PCC;
...
log.omega_g = omega_g + 1;
log.omega_conv = omega_conv + 1;
