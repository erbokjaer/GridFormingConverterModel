function dx = system_ode_6(t,x,p)
...
vg_d = vg_mag * cos(0 + vg_phase_rad);
vg_q = vg_mag * sin(0 + vg_phase_rad);
...
Econvdc = p.Kp_c*err_id + p.Ki_c*xi_id ...
      - omega_c*p.L1*i1q_c - p.L1*i1q_c + v_PCC_d_c;

Econvqc = p.Kp_c*err_iq + p.Ki_c*xi_iq ...
      + omega_c*p.L1*i1d_c + p.L1*i1d_c + v_PCC_q_c;
% Econvdc = p.Kp_c*err_id + p.Ki_c*xi_id ...
%       - omega_c*p.L1*Iq_ref - p.L1*Iq_ref + v_PCC_d_c;
% 
% Econvqc = p.Kp_c*err_iq + p.Ki_c*xi_iq ...
%       + omega_c*p.L1*Id_ref + p.L1*Id_ref + v_PCC_q_c;
